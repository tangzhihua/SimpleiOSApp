...
// 20150708 skyduck
uint isCellOneselfHandleSelectionCommand:1;
} delegateRespondsTo;
...
+ (instancetype) bindingHelperForTableView:(UITableView *)tableView
  ...
  return [[CETableViewBindingHelper alloc] initWithTableView:tableView
                                                sourceSignal:source
                                       selectionCommandArray:selectionArray
...
- (void)setDelegate:(id<UITableViewDelegate>)delegate {
  if (self.delegate != delegate) {
    ...
    // 20150708 skyduck
    newMethodCaching.isCellOneselfHandleSelectionCommand = ((CETableViewCell *)_templateCell).isOneselfHandleSelectionCommand;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  // skyduck 20150708
  ((CETableViewCell *)cell).selectionArray = _selectionArray;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  // execute the command
  if (!_delegateRespondsTo.isCellOneselfHandleSelectionCommand) {
    [[_selectionArray firstObject] execute:_data[indexPath.row]];
  }
