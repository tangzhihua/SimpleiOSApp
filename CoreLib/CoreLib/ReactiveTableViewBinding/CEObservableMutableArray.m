...
-(void)removeObjectAtIndex:(NSUInteger)index {
  // 20150704 skyduck
  id removedbject = _backingStore[index];
  if ([self.delegateForRemoved respondsToSelector:@selector(removedObject:)]) {
    [self.delegateForRemoved removedObject:removedbject];
  }
...
-(void)removeLastObject {
  // 20150704 skyduck
  id lastObject = [_backingStore lastObject];
  if ([self.delegateForRemoved respondsToSelector:@selector(removedObject:)]) {
    [self.delegateForRemoved removedObject:lastObject];
  }
